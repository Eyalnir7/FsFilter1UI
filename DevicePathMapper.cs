...
namespace FsFilter1UI
{
    public static class DevicePathMapper
    {
        ...
        public static string FromDevicePath(string path)
        {
            //its case sensetive and registry returns all caps so I make it normal again
            int index = path.IndexOfAny("0123456789".ToCharArray());
            string devicePath = @"\Device\HarddiskVolume" + path.Substring(index);
